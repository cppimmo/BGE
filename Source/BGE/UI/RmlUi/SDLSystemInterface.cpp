...
namespace BGE
{
	SDLSystemInterface::SDLSystemInterface(void)
	{
#if SDL_MAJOR_VERSION >= 3
		m_pCursorDefault = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_DEFAULT);
		m_pCursorMove = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_MOVE);
		m_pCursorPointer = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_POINTER);
		m_pCursorResize = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_NWSE_RESIZE);
		m_pCursorCross = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_CROSSHAIR);
		m_pCursorText = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_TEXT);
		m_pCursorUnavailable = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_NOT_ALLOWED);
#else
		m_pCursorDefault = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_ARROW);
		m_pCursorMove = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_SIZEALL);
		m_pCursorPointer = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_HAND);
		m_pCursorResize = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_SIZENWSE);
		m_pCursorCross = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_CROSSHAIR);
		m_pCursorText = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_IBEAM);
		m_pCursorUnavailable = SDL_CreateSystemCursor(SDL_SYSTEM_CURSOR_NO);
#endif
	}
	...
	{
#if SDL_MAJOR_VERSION >= 3
		auto destroyCursor = [](SDL_Cursor *pCursor) { SDL_DestroyCursor(pCursor); };
#else
		auto destroyCursor = [](SDL_Cursor *pCursor) { SDL_FreeCursor(pCursor); };
#endif
		destroyCursor(m_pCursorDefault);
		destroyCursor(m_pCursorMove);
		destroyCursor(m_pCursorPointer);
		destroyCursor(m_pCursorResize);
		destroyCursor(m_pCursorCross);
		destroyCursor(m_pCursorText);
		destroyCursor(m_pCursorUnavailable);
	}

	double SDLSystemInterface::GetElapsedTime(void)
	{
	...
	void SDLSystemInterface::SetMouseCursor(const Rml::String &cursorName)
	{
		SDL_Cursor *pCursor = nullptr;

		if (cursorName.empty() || cursorName == "arrow")
		{
			pCursor = m_pCursorDefault;
		}
		else if (cursorName == "move")
		{
			pCursor = m_pCursorMove;
		}
		else if (cursorName == "pointer")
		{
			pCursor = m_pCursorPointer;
		}
		else if (cursorName == "resize")
		{
			pCursor = m_pCursorResize;
		}
		else if (cursorName == "cross")
		{
			pCursor = m_pCursorCross;
		}
		else if (cursorName == "text")
		{
			pCursor = m_pCursorText;
		}
		else if (cursorName == "unavailable")
		{
			pCursor = m_pCursorUnavailable;
		}
	...
	{
		char *pRawText = SDL_GetClipboardText();
		text = Rml::String(pRawText);
		SDL_free(pRawText);
	}

	void SDLSystemInterface::ActivateKeyboard(Rml::Vector2f caretPosition, float lineHeight)
	{
		if (m_pWindow)
		{
#if SDL_MAJOR_VERSION >= 3
			const SDL_Rect rect = { static_cast<int>(caretPosition.x), static_cast<int>(caretPosition.y), 1, static_cast<int>(lineHeight) };
	...
	void SDLSystemInterface::DeactivateKeyboard(void)
	{
		if (m_pWindow)
		{
#if SDL_MAJOR_VERSION >= 3
	...
	void SDLSystemInterface::SetWindow(SDL_Window *pWindow)
	{
		m_pWindow = pWindow;
	}

	bool SDLSystemInterface::InputEventHandler(Rml::Context *pContext, SDL_Window *pWindow, SDL_Event &event)
	{
#if SDL_MAJOR_VERSION >= 3
...
#define RMLSDL_WINDOW_EVENTS_END \
	...
	break;
		...
		{
			result = pContext->ProcessMouseMove(static_cast<int>(event.motion.x), static_cast<int>(event.motion.y), GetKeyModifierState());
			break;
		}
		case event_mouse_down:
		{
			result = pContext->ProcessMouseButtonDown(ConvertMouseButton(event.button.button), GetKeyModifierState());
		...
		case event_mouse_up:
		{
			...
			result = pContext->ProcessMouseButtonUp(ConvertMouseButton(event.button.button), GetKeyModifierState());
			break;
		}
		case event_mouse_wheel:
		{
			result = pContext->ProcessMouseWheel(static_cast<float>(-event.wheel.y), GetKeyModifierState());
			break;
		}
		case event_key_down:
		{
			result = pContext->ProcessKeyDown(ConvertKey(getKey(event)), GetKeyModifierState());
			if (getKey(event) == SDLK_RETURN || getKey(event) == SDLK_KP_ENTER)
				result &= pContext->ProcessTextInput('\n');
			break;
		}
		case event_key_up:
		{
			result = pContext->ProcessKeyUp(ConvertKey(getKey(event)), GetKeyModifierState());
			break;
		}
		case event_text_input:
		{
			result = pContext->ProcessTextInput(Rml::String(&event.text.text[0]));
			break;
		}
		...
		case event_window_size_changed:
		{
			...
			pContext->SetDimensions(dimensions);
			break;
		}
		case event_window_leave:
		{
			pContext->ProcessMouseLeave();
			break;
		}
#if SDL_MAJOR_VERSION >= 3
		case SDL_EVENT_WINDOW_DISPLAY_SCALE_CHANGED:
		{
			const float display_scale = SDL_GetWindowDisplayScale(pWindow);
			pContext->SetDensityIndependentPixelRatio(display_scale);
			break;
		}
#endif
	...
	Rml::Input::KeyIdentifier SDLSystemInterface::ConvertKey(int sdlKey)
	{
#if SDL_MAJOR_VERSION >= 3
	...
	int SDLSystemInterface::ConvertMouseButton(int sdlMouseButton)
	{
	...
	int SDLSystemInterface::GetKeyModifierState(void)
	{
